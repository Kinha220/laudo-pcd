...
if st.button("Gerar PDF fixo"):
    ...
    texto(c, nome, 90, 682)
    texto(c, data_nascimento, 58, 650)
    texto(c, identidade, 120, 626)
    texto(c, orgao_emissor, 275, 627,14)
    texto(c, uf, 405, 627)
    texto(c, mae, 86, 603)
    texto(c, pai, 78, 578)
    texto(c, responsavel_legal, 212, 555)

    marcar(c, sexo == "Masculino", 260, 659)
    marcar(c, sexo == "Feminino", 377, 659)

    texto(c, cid_fisica, 199, 455)
    texto(c, sequelas_fisica, 372, 455)
    texto(c, cid_visual, 201, 430)
    texto(c, sequelas_visual, 372, 430)
